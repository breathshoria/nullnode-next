...
type DashboardProject = Pick<
  ProjectType,
  "id" | "title" | "description" | "logoUrl" | "onGoing"
>;
...
const Dashboard = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const dashboardProjects = response.data.map(
  ...
  if (isAddFormOpened) {
    return <AddProject toggleAddForm={toggleAddForm} />;
  }
  if (isEditFormOpened) {
    return (
      <EditProject
  ...
  if (isLoading) {
    return (
      <div
        className={
          "min-h-screen bg-gray-800 flex flex-col items-center justify-center"
        }
      >
  ...
  if (!session?.user.roles?.includes("admin")) {
    return (
      <div
        className={
          "bg-gray-800 h-screen mb-12 flex flex-col items-center justify-center"
        }
      >
  ...
  return (
    <div className="min-h-screen">
      ...
      <div
        className={"p-2 mt-5 flex flex-col gap-4 justify-center items-center"}
      >
      ...
      <div className={"flex flex-col justify-center items-center "}>
        <button
          className={"p-2 bg-sky-700 rounded-lg m-3 font-medium w-16"}
