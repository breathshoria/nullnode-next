...
type Props = Pick<
  ProjectType,
  "id" | "logoUrl" | "title" | "description" | "onGoing"
>;
    ...
    <Link href={`/projects/${id}`}>
      ...
      >
        <img
          ...
          src={`${process.env.NEXT_PUBLIC_API}/${logoUrl}`}
        />
        <span className={"py-2 inline-block w-full text-center font-semibold"}>
          {title}
        </span>
        <p
          className={
            "inline-block w-full text-start bg-gray-800 rounded-xl p-3"
          }
        >
          {description}
        </p>
